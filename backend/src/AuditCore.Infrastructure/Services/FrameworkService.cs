...
public sealed class FrameworkService : IFrameworkService
{
    ...
    public FrameworkService(IAuditCoreDbContext dbContext) => _dbContext = dbContext;
    ...
    public async Task<FrameworkDto> CreateFrameworkAsync(CreateFrameworkRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var exists = await _dbContext.ControlFrameworks.AnyAsync(x => x.Code == code && x.Version == request.Version.Trim(), cancellationToken);
        if (exists) throw new InvalidOperationException("Ya existe esta versión del marco de control.");
        var entity = new ControlFramework(request.Name, code, request.Version, request.Description);
    ...
    public async Task<FrameworkDto?> UpdateFrameworkAsync(Guid id, UpdateFrameworkRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var code = request.Code.Trim().ToUpperInvariant();
        var duplicate = await _dbContext.ControlFrameworks.AnyAsync(x => x.Id != id && x.Code == code && x.Version == request.Version.Trim(), cancellationToken);
        if (duplicate) throw new InvalidOperationException("Ya existe esta versión del marco de control.");
        entity.Update(request.Name, code, request.Version, request.Description, request.IsActive);
        await _dbContext.SaveChangesAsync(cancellationToken);
        return new FrameworkDto(entity.Id, entity.Name, entity.Code, entity.Version, entity.Description, entity.IsActive);
    }
    ...
    public async Task<IReadOnlyCollection<EvaluationDto>> GetEvaluationsAsync(Guid auditId, CancellationToken cancellationToken = default) =>
        await _dbContext.ControlEvaluations.AsNoTracking().Where(x => x.AuditId == auditId).OrderBy(x => x.Control.Code)
    ...
    public async Task<EvaluationDto> EvaluateAsync(Guid auditId, Guid controlId, EvaluateControlRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await _dbContext.SaveChangesAsync(cancellationToken);
        return new EvaluationDto(entity.Id, entity.AuditId, entity.ControlId,
            await _dbContext.ControlDefinitions.Where(x => x.Id == controlId).Select(x => x.Code).SingleAsync(cancellationToken),
            entity.Score, entity.Status, entity.Notes, entity.EvaluatedByUserId, entity.EvaluatedAtUtc);
    }
}
