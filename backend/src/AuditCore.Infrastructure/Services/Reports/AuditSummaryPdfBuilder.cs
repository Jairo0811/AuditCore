...
internal static class AuditSummaryPdfBuilder
{
    ...
    public static byte[] Build(IReadOnlyCollection<string[]> rows)
    {
        ...
        var logo = LoadLogo();
        var logoScale = Math.Min(LogoMaxWidth / logo.Width, LogoMaxHeight / logo.Height);
    ...
    {
        var logoPath = Path.Combine(AppContext.BaseDirectory, "Assets", "Brand", "auditcore-logo.png");
        if (!File.Exists(logoPath))
        {
            throw new FileNotFoundException($"No se encontró el logo oficial de AuditCore en '{logoPath}'.", logoPath);
        }
        ...
        double logoHeight)
    {
        ...
        DrawRectangle(content, 0, HeaderBottom, PageWidth, HeaderHeight, "0.027 0.094 0.165");
        DrawRectangle(content, 0, HeaderDividerY, PageWidth, 2, "0.078 0.722 0.651");
        DrawImage(content, "ImLogo", LogoX, LogoY, logoWidth, logoHeight);
        DrawText(content, "Resumen de auditorías", TitleX, TitleY, 18, bold: true, color: "0.918 0.949 0.976");
        DrawText(content, "Reporte ejecutivo consolidado", TitleX, SubtitleY, 9.2, color: "0.663 0.741 0.816");

        var generatedAt = DateTimeOffset.Now.ToString("dd/MM/yyyy HH:mm", CultureInfo.InvariantCulture);
        DrawText(content, $"Generado: {generatedAt}", RightPanelX, GeneratedAtY, 8.5, color: "0.663 0.741 0.816");
        DrawMetric(content, RightPanelX, MetricY, MetricWidth, MetricHeight, totalRows);

        if (totalRows == 0) DrawEmptyState(content);
        else DrawTable(content, rows);

        DrawFooter(content, pageNumber, pageCount);
    ...
    private static void DrawMetric(StringBuilder content, double x, double y, double width, double height, int totalRows)
    {
        ...
        DrawRectangleStroke(content, x, y, width, height, "0.102 0.196 0.286", 0.7);
        DrawLine(content, x, y, x + width, y, "0.078 0.722 0.651", 1.5);
        DrawText(content, "TOTAL DE AUDITORÍAS", x + 12, y + 26, 7.6, bold: true, color: "0.557 0.647 0.741");
        DrawText(content, totalRows.ToString(CultureInfo.InvariantCulture), x + 12, y + 7, 17, bold: true, color: "0.345 0.918 0.831");
    }

    private static void DrawEmptyState(StringBuilder content)
    {
        var width = PageWidth - (Margin * 2);
        DrawRectangle(content, Margin, EmptyStateY, width, EmptyStateHeight, "0.039 0.125 0.212");
        DrawRectangleStroke(content, Margin, EmptyStateY, width, EmptyStateHeight, "0.122 0.224 0.329", 1);
        DrawLine(content, Margin + 230, EmptyStateY + 117, PageWidth - Margin - 230, EmptyStateY + 117, "0.078 0.722 0.651", 1);
        DrawText(content, "SIN DATOS PARA MOSTRAR", 300, EmptyStateY + 96, 13.5, bold: true, color: "0.345 0.918 0.831");
        DrawText(content, "No hay auditorías disponibles para los filtros seleccionados.", 236, EmptyStateY + 67, 10, color: "0.663 0.741 0.816");
        DrawText(content, "El reporte se actualizará automáticamente cuando existan registros.", 220, EmptyStateY + 47, 9, color: "0.459 0.553 0.647");
    }
    ...
    private static void DrawTable(StringBuilder content, IReadOnlyCollection<string[]> rows)
    {
        ...
        var x = Margin;
        var headerBottom = TableTop - HeaderRowHeight;

        DrawRectangle(content, Margin, headerBottom, tableWidth, HeaderRowHeight, "0.039 0.157 0.255");
        DrawRectangleStroke(content, Margin, headerBottom, tableWidth, HeaderRowHeight, "0.078 0.722 0.651", 0.9);
    ...
    private static byte[] BuildPdfDocument(IReadOnlyList<string> pageStreams, PngImageData logo)
    {
        var pageCount = pageStreams.Count;
        var regularFontObjectId = 3 + (pageCount * 2);
        var boldFontObjectId = regularFontObjectId + 1;
        var logoObjectId = boldFontObjectId + 1;
        var alphaObjectId = logo.HasAlpha ? logoObjectId + 1 : (int?)null;
        var pageObjectIds = Enumerable.Range(0, pageCount).Select(index => 3 + (index * 2)).ToArray();
        var objects = new List<PdfObject>();

        objects.Add(PdfObject.FromText("<< /Type /Catalog /Pages 2 0 R >>"));
        var pageReferences = string.Join(" ", pageObjectIds.Select(id => $"{id} 0 R"));
        objects.Add(PdfObject.FromText($"<< /Type /Pages /Kids [{pageReferences}] /Count {pageCount} >>"));

        for (var index = 0; index < pageCount; index++)
        {
            var contentObjectId = pageObjectIds[index] + 1;
            var pageResources = $"<< /Font << /F1 {regularFontObjectId} 0 R /F2 {boldFontObjectId} 0 R >> /XObject << /ImLogo {logoObjectId} 0 R >> >>";
            objects.Add(PdfObject.FromText($"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 {PageWidth:0} {PageHeight:0}] /Resources {pageResources} /Contents {contentObjectId} 0 R >>"));
            var streamBytes = Encoding.ASCII.GetBytes(pageStreams[index]);
            objects.Add(PdfObject.FromStream(streamBytes, string.Empty));
        }

        objects.Add(PdfObject.FromText("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>"));
        objects.Add(PdfObject.FromText("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>"));

        var alphaReference = alphaObjectId.HasValue ? $" /SMask {alphaObjectId.Value} 0 R" : string.Empty;
        objects.Add(PdfObject.FromStream(
            logo.RgbData,
            $"/Type /XObject /Subtype /Image /Width {logo.Width} /Height {logo.Height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /FlateDecode{alphaReference}"));
        ...
        for (var index = 1; index < offsets.Count; index++) WriteAscii(output, $"{offsets[index]:0000000000} 00000 n \n");
        WriteAscii(output, $"trailer << /Size {objects.Count + 1} /Root 1 0 R >>\nstartxref\n{xrefPosition}\n%%EOF");
        return output.ToArray();
    }

    private static void DrawImage(StringBuilder content, string resourceName, double x, double y, double width, double height)
    {
        content.Append("q ").Append(Number(width)).Append(" 0 0 ").Append(Number(height)).Append(' ')
            .Append(Number(x)).Append(' ').Append(Number(y)).Append(" cm /").Append(resourceName).Append(" Do Q\n");
    }
        ...
        string color = "1 1 1")
    {
        ...
        content.Append("BT /").Append(bold ? "F2" : "F1").Append(' ').Append(Number(fontSize)).Append(" Tf ")
            .Append(Number(x)).Append(' ').Append(Number(y)).Append(" Td (").Append(EscapePdfText(value)).Append(") Tj ET\n");
    }
    ...
    private static void DrawRectangle(StringBuilder content, double x, double y, double width, double height, string color)
    {
    ...
    private static void DrawRectangleStroke(StringBuilder content, double x, double y, double width, double height, string color, double lineWidth)
    {
    ...
    private static void DrawLine(StringBuilder content, double x1, double y1, double x2, double y2, string color, double lineWidth)
    {
        ...
        foreach (var valueByte in bytes)
        {
            switch (valueByte)
            {
                ...
                default:
                    ...
                    else result.Append((char)valueByte);
                    break;
            }
        }
    ...
    {
        public static PdfObject FromText(string value) => new(Encoding.ASCII.GetBytes(value));

        public static PdfObject FromStream(byte[] data, string dictionary)
        {
            using var output = new MemoryStream();
            WriteAscii(output, $"<< {dictionary} /Length {data.Length} >>\nstream\n");
    ...
    private sealed record PngImageData(int Width, int Height, byte[] RgbData, byte[]? AlphaData)
    {
        public bool HasAlpha => AlphaData is not null;

        public static PngImageData Load(string path)
        {
            var png = File.ReadAllBytes(path);
            ReadOnlySpan<byte> signature = [137, 80, 78, 71, 13, 10, 26, 10];
            if (png.Length < signature.Length || !png.AsSpan(0, signature.Length).SequenceEqual(signature))
                throw new InvalidDataException("El archivo configurado como logo no es un PNG válido.");
            ...
            while (position + 12 <= png.Length)
            {
                ...
                position += 4;
                var chunkType = Encoding.ASCII.GetString(png, position, 4);
                position += 4;

                if (length < 0 || position + length + 4 > png.Length)
                    throw new InvalidDataException("El PNG del logo contiene un chunk inválido.");

                var chunkData = png.AsSpan(position, length);
                switch (chunkType)
                {
                    case "IHDR":
            ...
            if (width <= 0 || height <= 0) throw new InvalidDataException("No fue posible obtener las dimensiones del logo PNG.");
            if (bitDepth != 8) throw new NotSupportedException("El logo PNG debe utilizar 8 bits por canal.");
            if (colorType is not 2 and not 6) throw new NotSupportedException("El logo PNG debe estar en formato RGB o RGBA.");
            if (interlace != 0) throw new NotSupportedException("El logo PNG no puede estar entrelazado.");

            var bytesPerPixel = colorType == 6 ? 4 : 3;
            var stride = checked(width * bytesPerPixel);
            var expectedSize = checked(height * (stride + 1));
            ...
            if (filtered.Length < expectedSize) throw new InvalidDataException("El contenido del PNG está incompleto.");

            var reconstructed = new byte[checked(height * stride)];
            ...
            {
                var filterType = filtered[sourceOffset++];
                Array.Copy(filtered, sourceOffset, currentRow, 0, stride);
                sourceOffset += stride;
                ApplyPngFilter(currentRow, previousRow, bytesPerPixel, filterType);
                Buffer.BlockCopy(currentRow, 0, reconstructed, row * stride, stride);
                (previousRow, currentRow) = (currentRow, previousRow);
            }

            var rgb = new byte[checked(width * height * 3)];
            byte[]? alpha = colorType == 6 ? new byte[checked(width * height)] : null;
            var sourceIndex = 0;
            var rgbIndex = 0;
            var alphaIndex = 0;
            ...
            {
                rgb[rgbIndex++] = reconstructed[sourceIndex++];
                rgb[rgbIndex++] = reconstructed[sourceIndex++];
                rgb[rgbIndex++] = reconstructed[sourceIndex++];
                if (colorType == 6) alpha![alphaIndex++] = reconstructed[sourceIndex++];
            }

            return new PngImageData(width, height, Compress(rgb), alpha is null ? null : Compress(alpha));
        }

        private static void ApplyPngFilter(byte[] current, byte[] previous, int bytesPerPixel, byte filterType)
        {
            for (var index = 0; index < current.Length; index++)
            {
                ...
                current[index] = filterType switch
                {
                    0 => current[index],
                    1 => unchecked((byte)(current[index] + left)),
                    2 => unchecked((byte)(current[index] + above)),
                    3 => unchecked((byte)(current[index] + ((left + above) / 2))),
                    4 => unchecked((byte)(current[index] + PaethPredictor(left, above, upperLeft))),
                    _ => throw new InvalidDataException($"Filtro PNG no soportado: {filterType}.")
                };
            }
        }
        ...
        private static byte PaethPredictor(int left, int above, int upperLeft)
        {
            ...
            if (distanceLeft <= distanceAbove && distanceLeft <= distanceUpperLeft) return (byte)left;
            if (distanceAbove <= distanceUpperLeft) return (byte)above;
